...
TEST(Metrics, EstimateNetHeight) {
    auto params = RegtestActivateHeartwood(false, 200);
...
TEST(Metrics, NextUpgrade) {
    ...
    EXPECT_EQ(SecondsLeftToNextEpoch(params, 1).value(), 11880);
    EXPECT_EQ(DisplayDuration(SecondsLeftToNextEpoch(params, 1).value(), DurationFormat::REDUCED), "3 hours");
    EXPECT_EQ(DisplayDuration(SecondsLeftToNextEpoch(params, 1).value(), DurationFormat::FULL), "3 hours, 18 minutes, 0 seconds");

    EXPECT_EQ(SecondsLeftToNextEpoch(params, 90).value(), 1200);
    EXPECT_EQ(DisplayDuration(SecondsLeftToNextEpoch(params, 90).value(), DurationFormat::REDUCED), "20 minutes");
    EXPECT_EQ(DisplayDuration(SecondsLeftToNextEpoch(params, 90).value(), DurationFormat::FULL), "20 minutes, 0 seconds");

    EXPECT_EQ(SecondsLeftToNextEpoch(params, 99).value(), 120);
    EXPECT_EQ(DisplayDuration(SecondsLeftToNextEpoch(params, 99).value(), DurationFormat::REDUCED), "2 minutes");
    EXPECT_EQ(DisplayDuration(SecondsLeftToNextEpoch(params, 99).value(), DurationFormat::FULL), "2 minutes, 0 seconds");
